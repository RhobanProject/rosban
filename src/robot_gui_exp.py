...
from dynamixel_hardware_interface.msg import * \
    # import dynamixel_msgs node's messages
...
class controllerGUI(wx.Frame):
    ...
    def __init__(self, parent, debug=False, default_enabled=False):
        ...
        # for name in sorted(joints.keys()):
            ...
            init_angle_raw = self.dynamixels[name]['motors'][0]['init']
            # print params
            min_angle = (self.dynamixels[name]['motors'][0]['min'] - init_angle_raw) / self.params[
                int(self.dynamixels[name]['motors'][0]['id'])]['encoder_ticks_per_radian']
            max_angle = (self.dynamixels[name]['motors'][0]['max'] - init_angle_raw) / self.params[
                int(self.dynamixels[name]['motors'][0]['id'])]['encoder_ticks_per_radian']
            ...
            init_angle = self.dynamixels[name]['motors'][0]['init'] / self.params[
                self.dynamixels[name]['motors'][0]['id']]['encoder_ticks_per_radian']
            self.id_name[self.dynamixels[name]['motors'][0]['id']] = name
            self.name_id[self.dynamixels[name]['joint']] = self.dynamixels[
                name]['motors'][0]['id']
            ...
            # create publisher
            self.publishers[self.dynamixels[name]['motors'][0]['id']] = rospy.Publisher(
            ...
            # ROS it still works)
            self.subscribers[self.dynamixels[name]['motors'][0]['id']] = rospy.Subscriber(
                name + '/state', JointState, self.stateCb)

            # create service proxy
            self.services[self.dynamixels[name]['motors'][0]['id']] = rospy.ServiceProxy(
                name + '/set_torque_limit', SetTorqueLimit)

            self.services[
                self.dynamixels[name]['motors'][0]['id']](INIT_TORQUE)
            ...
            # else:
            #     self.services[
            #         self.dynamixels[name]['motors'][0]['id']](ON_TORQUE)

            # create slider
            # s = servoSlider(self, min_angle, max_angle, name, i)
            s = servoSlider(self, min_angle, max_angle, self.dynamixels[
                            name]['joint'], self.dynamixels[name]['motors'][0]['id'], default_enabled)
            ...
            # self.labels_objs[self.dynamixels[name]['motors'][0]['id']] = wx.StaticText(
            #     self, label=str(s.getPosition()))

            self.labels_objs[self.dynamixels[name]['joint']] = wx.StaticText(
                self, label=str(s.getPosition()))

            self.text_entry[self.dynamixels[name]['motors'][0]['id']] = wx.TextCtrl(
                self, self.dynamixels[name]['motors'][0]['id'], size=(-1, -1), style=wx.TE_PROCESS_ENTER)

            # self.labels[self.dynamixels[name]['motors'][0]['id']] = ""
            self.labels[self.dynamixels[name]['joint']] = ""
            ...
            servoSizer.Add(self.labels_objs[self.dynamixels[name]['joint']], (
                i, 2), wx.DefaultSpan, wx.ALIGN_CENTER_VERTICAL)

            servoSizer.Add(self.text_entry[self.dynamixels[name]['motors'][0]['id']], (
                i, 4), wx.DefaultSpan, wx.ALIGN_CENTER_VERTICAL)

            self.servos[self.dynamixels[name]['motors'][0]['id']] = s
            # self.servos_pos[
            #     self.dynamixels[name]['motors'][0]['id']] = init_angle

            self.servos_pos[
                self.dynamixels[name]['joint']] = init_angle
    ...
    def rad_to_raw(self, motor_id, angle):
        ...
        initial_position_raw = self.dynamixels[name]['motors'][0]['init']

        angle_raw = angle * \
            self.params[int(self.dynamixels[name]['motors'][0]['id'])][
                'encoder_ticks_per_radian']

        flipped = self.dynamixels[name]['motors'][0][
            'min'] > self.dynamixels[name]['motors'][0]['max']
    ...
    def stateCb(self, msg):
        ...
        self.servos_pos[msg.name] = msg.position
        # deg
        # self.servos_pos[msg.motor_ids[0]] = degrees(msg.position)

        # self.labels[msg.motor_ids[0]].SetLabel('%.3f'%(msg.position))
        self.labels[msg.name] = "%.3f (%d)" % (
            msg.position, self.rad_to_raw(self.name_id[msg.name], msg.position))
        # self.labels[msg.motor_ids[0]] = "%.3f" % (degrees(msg.position))

            # self.labels[msg.motor_ids[0]].SetLabel('%.3f'%(msg.position))
            # print '%.3f'%(msg.position)
